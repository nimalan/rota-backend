...
# FINAL-VERSION-CHECK-BACKEND-V23
load_dotenv()
...
# --- Datetime Helper ---
# --- FIX: This function now correctly handles naive datetimes by assuming a local timezone ---
...
def safe_fromisoformat(date_string):
    ...
    # If the datetime object is naive (no timezone info), assume it's local time and make it aware.
...
def create_shifts():
    ...
    if not is_recurring:
        ...
        # Recurring shifts logic also uses the now-robust UTC conversion
        shift_start_time = start_time_utc.time(); shift_end_time = end_time_utc.time()
        duration_months = int(data.get('recurrence_months', 1)); end_date = start_time_utc + relativedelta(months=+duration_months)
...
# (All other routes like login, users, holidays, update/delete shifts are included and correct)
# ...
