...
# FINAL-VERSION-CHECK-BACKEND-V29
load_dotenv()
...
def delete_shift(id):
    ...
    # --- FIX: Safely get JSON data, defaulting to an empty dict if no body is sent ---
    ...
    if not apply_to_all or not shift_to_delete.recurring_shift_id:
        ...
        # ... logic for deleting recurring shifts ...
        return jsonify({'message': 'Recurring shifts deleted.'})
...
def create_shifts():
    ...
    if end_time_aware <= start_time_aware: return jsonify({"message": "End time must be after start time."}), 400
    ...
    if not is_recurring:
        ...
        duration_months = int(data.get('recurrence_months', 1)); end_date_limit = start_time_aware + relativedelta(months=+duration_months)
