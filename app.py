...
class Shift(db.Model):
    ...
    # --- NEW: Link recurring shifts together ---
    # This ID will be shared by all shifts in the same recurring series.
...
def get_shifts():
    ...
    # The simple GET request now just returns all shifts in a range.
    # The logic for recurring shifts now happens during creation (POST).
...
def create_shifts():
    ...
    if not is_recurring:
        ...
        # --- NEW: Logic for creating a series of recurring shifts ---
        ...
        # Generate a unique ID for this entire series of shifts
        ...
        db.session.commit()
        # ... bulk notification logic could be added here ...
...
def update_shift(id):
    ...
    if not apply_to_all or not shift_to_update.recurring_shift_id:
        ...
        # Detach it from the recurring series if it was part of one
        shift_to_update.recurring_shift_id = None 
        db.session.commit()
        return shift_schema.jsonify(shift_to_update)
    else:
        # --- NEW: Logic to update all future shifts in a series ---
...
def delete_shift(id):
    ...
    if not apply_to_all or not shift_to_delete.recurring_shift_id:
        ...
        # --- NEW: Logic to delete all future shifts in a series ---
