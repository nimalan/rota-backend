...
# FINAL-VERSION-CHECK-BACKEND-V24
load_dotenv()
...
if db_url:
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(
        os.path.abspath(os.path.dirname(__file__)), 'rota.db'
    )
...
def get_shifts():
    ...
    if not start_date_str or not end_date_str:
        return jsonify({"message": "Date range (start_date and end_date) required"}), 400
    ...
    return shifts_schema.jsonify(shifts)
...
def create_shifts():
    ...
    if not is_recurring:
        ...
        shift_start_time = start_time_utc.time()
        shift_end_time = end_time_utc.time()
        ...
        db.session.commit()
        return shifts_schema.jsonify(created_shifts), 201
...
def update_shift(id):
    ...
    if not apply_to_all or not shift_to_update.recurring_shift_id:
        shift_to_update.start_time = new_start_dt
        shift_to_update.end_time = new_end_dt
        ...
        db.session.commit()
        return shifts_schema.jsonify(future_shifts)
...
def delete_shift(id):
    ...
    if not apply_to_all or not shift_to_delete.recurring_shift_id:
        ...
        future_shifts = Shift.query.filter(
            Shift.recurring_shift_id == recurring_id,
            Shift.start_time >= shift_to_delete.start_time
        ).all()
        for shift in future_shifts:
...
def get_holidays():
    ...
    return holidays_schema.jsonify(holidays)

@app.route('/holidays', methods=['POST'])
def request_holiday():
    data = request.get_json()
    if not data or not all(k in data for k in ('user_id', 'start_date', 'end_date')):
        return jsonify({"message": "user_id, start_date, and end_date are required."}), 400
