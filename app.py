...
# FINAL-VERSION-CHECK-BACKEND-V26
load_dotenv()
...
class HolidaySchema(ma.SQLAlchemyAutoSchema):
    start_date = fields.Date(format='iso'); end_date = fields.Date(format='iso')
    class Meta:
...
def create_shifts():
    data = request.get_json(); start_time_utc = safe_fromisoformat(data['start_time']); end_time_utc = safe_fromisoformat(data['end_time'])
    if end_time_utc <= start_time_utc: return jsonify({"message": "End time must be after start time."}), 400
    ...
    if not is_recurring:
        ...
        shift_start_time = start_time_utc.time(); shift_end_time = end_time_utc.time()
        duration_months = int(data.get('recurrence_months', 1)); end_date_limit = start_time_utc + relativedelta(months=+duration_months)
...
def delete_shift(id):
    shift_to_delete = Shift.query.get_or_404(id); data = request.get_json(silent=True) or {}; apply_to_all = data.get('apply_to_all', False)
